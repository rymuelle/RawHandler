...
class RawHandler():
    ...
    def __init__(self, path):
        ...
        assert self.rawpy.color_desc.decode() == 'RGBG', 'Only raw files with Bayer patters are supported currently.'
    ...
    def remove_masked_pixels(self, img):
        return img[:, 0:raw.sizes.iheight, 0:raw.sizes.iwidth]
    ...
    def input_handler(self, dims=None, img=None):
        ...
        if dims is not None:
            if len(dims) != 4:
                raise ValueError(f'Arguments must be length 0 or 4, found length {dims}.')
    ...
    def adjust_bayer_bw_levels(self, bayer):
        ...
        for channel in range(4):
            ...
            bayer[bayer_map==channel] *= 1.0 / (self.rawpy.white_level - self.rawpy.black_level_per_channel[channel])
        return bayer
    ...
    def as_rggb(self, dims=None, img=None):
        ...
        def get_matching_index(channel):
            return [idx for idx in range(len(self.bayer_pattern)) if self.bayer_pattern[idx]== channel]
        r_idx = get_matching_index('R')
        g_idx = get_matching_index('G')
        b_idx = get_matching_index('B')
        assert (len(r_idx) == 1) and (len(g_idx) == 2) and (len(b_idx) == 1), 'Incorrect number of channels found.'
        smart_indexing_array = ((0, 0), (0, 1), (1, 0), (1, 1))
        rggb = np.stack([raw[0, idxs[1]::2, idxs[0]::2] for idxs in smart_indexing_array])
        return rggb
    ...
    def as_rgb(self, dims=None, img=None):
        ...
        '''
        color_code = {'RGGB': cv2.COLOR_BayerRGGB2RGB_EA, 
                      'GRBG': cv2.COLOR_BayerGRBG2RGB_EA,
                      'BGGR': cv2.COLOR_BayerBGGR2RGB_EA,
                      'GBRG': cv2.COLOR_BayerGBRG2RGB_EA}
        assert self.bayer_pattern in ['RGGB', 'GRBG', 'BGGR', 'GBRG'], f'{self.bayer_pattern} is not supported for demosaicing.'
    ...
    def as_RGB_colorspace(self, dims=None, img=None, xyz_to_colorspace=None, colorspace=None):
        '''
        ...
        rgb_to_xyz = np.linalg.inv(raw.rgb_xyz_matrix[:3])
        ...
        assert xyz_to_colorspace is not None, 'Color space not supported, please supply color space.'
