...
from RawHandler.utils import make_colorspace_matrix, transform_colorspace_to_rggb, pixel_unshuffle, pixel_shuffle
...
class BaseRawHandler:
    ...
    def __init__(self, pixel_array: np.ndarray, core_metadata: CoreRawMetadata, full_metadata: Optional[dict] = None, 
                 colorspace: Literal['camera', 'XYZ', 'sRGB', 'AdobeRGB', 'lin_rec2020']='lin_rec2020'):
    ...
    def generate_thumbnail(self, min_preview_size=256, colorspace=None, clip=False, demosaicing_func=demosaicing_CFA_Bayer_bilinear) -> np.ndarray:
        img = self.downsize(min_preview_size=min_preview_size, colorspace=colorspace, clip=clip)
...
class RawHandler:
    ...
    def __new__(cls, path: str, **kwargs):
        ...
        return BaseRawHandler(pixel_array=raw_image, core_metadata=core_metadata, full_metadata=metadata, **kwargs)